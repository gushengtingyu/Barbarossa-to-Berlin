...
test("generated map data contains only valid endpoints", () => {
	...
	assert.equal(data.edges.length, 638)
	assert.equal(
...
test("printed map symbols and special attack restrictions are represented as authored facts", () => {
	...
	assert.deepEqual(Object.fromEntries(["regular", "river", "sr"].map((type) => [type, data.edges.filter((edge) => edge.type === type).length])), { regular: 529, river: 71, sr: 38 })
