...
test("map attributes cover every space and preserve combined terrain symbols", () => {
	...
	assert.equal(data.spaces[343].name, "Dnepropetrovsk")
	assert.equal(data.spaces[343].urban, true)
	assert.equal(data.spaces[328].name, "Krasnodar")
	assert.equal(data.spaces[328].urban, false)
	assert.equal(data.spaces[89].name, "Vienna")
